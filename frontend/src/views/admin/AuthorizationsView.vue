...
const roomId = ref('')
const rows = ref<RoomAuthorization[]>([])
const userId = ref('')
...
async function load(): Promise<void> {
  ...
  loading.value = true
  try { rows.value = await adminApi.authorizations(roomId.value); page.value = 1 } finally { loading.value = false }
}
...
<template>
  <section class="admin-page">
    <div class="page-heading"><div><h2>管理员授权</h2><p>仅 SYSTEM_ADMIN 可管理。授权不会提升用户角色，只赋予单一房间的运营权限。</p></div></div>
    <el-card shadow="never"><el-form :inline="true"><el-form-item label="聊天室"><el-select v-model="roomId" filterable placeholder="请选择房间" @change="load"><el-option v-for="room in rooms" :key="room.id" :label="room.name" :value="room.id"/></el-select></el-form-item><el-form-item label="管理员用户 ID"><el-input v-model="userId" placeholder="UUID"/></el-form-item><el-button type="primary" :disabled="!roomId || !userId.trim()" @click="grant">添加授权</el-button></el-form></el-card>
    <el-card shadow="never" class="table-card">
      <el-empty v-if="!roomId" description="请选择一个房间"/>
      <template v-else>
        ...
        <TablePagination :page="page" :page-size="pageSize" :item-count="visibleRows.length" :has-next="page * pageSize < rows.length" @change="page = $event"/>
      </template>
    </el-card>
  </section>
</template>

<style scoped>.page-heading{margin-bottom:18px}.page-heading h2{margin:0 0 6px}.page-heading p{margin:0;color:var(--el-text-color-secondary)}.table-card{margin-top:16px}</style>
