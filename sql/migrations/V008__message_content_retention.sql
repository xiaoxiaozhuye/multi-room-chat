-- Message retention removes only the body.  The immutable message identity,
-- ordering sequence and all foreign-key targets remain available to audit and
-- forensic queries.
...
-- This is operational evidence for a retention run.  It is deliberately
-- separate from audit_logs: audit records have their own, longer policy and
-- are never selected by the message-body retention job.
