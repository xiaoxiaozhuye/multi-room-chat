...
const authorizationPage = ref(1); const authorizationPageSize = 20
const visibleAuthorizations = computed(() => authorizations.value.slice((authorizationPage.value - 1) * authorizationPageSize, authorizationPage.value * authorizationPageSize))
...
async function load(): Promise<void> {
  loading.value = true
  try { const response = await adminApi.rooms({ ...filters, page: page.value, size: pageSize }); rooms.value = response.items; hasNext.value = response.hasNext } finally { loading.value = false }
}
...
async function open(room: AdminRoom): Promise<void> {
  ...
  if (systemAdmin.value) { authorizations.value = await adminApi.authorizations(room.id); authorizationPage.value = 1 }
}
...
async function grant(): Promise<void> {
  ...
  authorizations.value = await adminApi.authorizations(editing.value.id); authorizationPage.value = 1; ElMessage.success('授权已生效')
}
async function revoke(item: RoomAuthorization): Promise<void> {
  ...
  await adminApi.revoke(editing.value.id, item.adminUserId); authorizations.value = await adminApi.authorizations(editing.value.id); authorizationPage.value = Math.min(authorizationPage.value, Math.max(1, Math.ceil(authorizations.value.length / authorizationPageSize)))
}
onMounted(load)
</script>
...
<template>
  <section class="admin-page">
    ...
    </el-table><div class="mobile-card-list"><div v-for="room in rooms" :key="room.id" class="mobile-card" @click="open(room)"><header><strong>{{ room.name }}</strong><el-tag :type="tagType(room.status)">{{ room.status }}</el-tag></header><div class="mobile-card-meta"><span>{{ room.joinMode === 'OPEN' ? '开放加入' : '审批加入' }} · 成员上限 {{ room.maxMembers }}</span><span>最后更新：{{ time(room.updatedAt) }}</span></div><div class="mobile-card-actions"><el-button type="primary" plain @click.stop="open(room)">查看管理</el-button></div></div><el-empty v-if="!loading && !rooms.length" description="暂无可管理的聊天室" /></div><TablePagination :page="page" :page-size="pageSize" :item-count="rooms.length" :has-next="hasNext" @change="changePage" /></el-card>
