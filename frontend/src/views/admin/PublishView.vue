...
const resultPage = ref(1); const resultPageSize = 20
const visibleResults = computed(() => results.value.slice((resultPage.value - 1) * resultPageSize, resultPage.value * resultPageSize))
...
async function send(): Promise<void> {
  ...
  sending.value = true; try { const response = await adminApi.publish(emergency, targetIds.value, content.value.trim()); results.value = response.results; resultPage.value = 1; ElMessage.success('请求已处理，请查看逐房间结果') } finally { sending.value = false }
}
